...
NSString *const SDWebImageManagerProgressNotificationInfoImageURL = @"image_url";
...
- (void)reportProgressForDelegate:(id <SDWebImageManagerDelegate>)delegate progress:(CGFloat)progress url:(NSURL*)url{
    NSDictionary* dic = [NSDictionary dictionaryWithObjectsAndKeys:
                         [NSNumber numberWithFloat:progress], SDWebImageManagerProgressNotificationInfoProgressKey,
                         url, SDWebImageManagerProgressNotificationInfoImageURL,
                         nil];
    //NSLog(@"!!!!!!!!%@", dic);
...
//    NSLog(@"############ progress:1 %@", [url description]);
    [self reportProgressForDelegate:delegate progress:1 url:url];
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if (!downloader)
    {
//        NSLog(@"############ progress:0 %@", [url description]);

        [self reportProgressForDelegate:delegate progress:0 url:url];
...
//        NSLog(@"############ progress:$$$ %@", [url description]);

        downloader.lowPriority = (options & SDWebImageLowPriority);
        [self reportProgressForDelegate:delegate progress:(downloader.totalReceivedLength / downloader.expectedContentLength) url:downloader.url];
    }
...
- (void)imageDownloaderDidReceiveData:(SDWebImageDownloader *)downloader {
    ...
    for (NSInteger idx = [downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        {
//            NSLog(@"############ progress:&&&& %@", [aDownloader.url description]);

            id<SDWebImageManagerDelegate> delegate = [downloadDelegates objectAtIndex:idx];
            [self reportProgressForDelegate:delegate progress:(downloader.totalReceivedLength / downloader.expectedContentLength) url:aDownloader.url];
        }
    }
}
